...
reverse=0
turn=0
kp,ki,kd=0.03,0.0,0.005#kp=0.01
kp1,kd1,ki1,I=0.03,0.0,0.0,0
...
class PubNode:
    ...
    def wheel_speed(self,err,ang_err):
        global kp,kd,p_err,ki,I
        base_speed = 0.8
        D = (err-ang_err)-p_err
        I=I+(err-ang_err)
        err_speed=kp*(err-ang_err)+kd*D+ki*I
        p_err = err-ang_err
    ...
    def wheel_speed_hr(self,err,ang_err):
        global kp1,kd1,p_err,ki1,I
        base_speed = 0.6
        D = (err-ang_err)-p_err
        I=I+(err-ang_err)
        err_speed=kp1*(err)+kd1*D+ki1*I
        p_err = err-ang_err
        self.msg.linear.x = base_speed    #0.1
        self.msg.linear.y = 0.0
        self.msg.linear.z = 0.0
        ...
        self.msg.angular.y = 0.0
        self.msg.angular.x = 0.0
        #print("l_err = "+str(err)+"and ang = "+str(ang_err))
    ...
    def bot_error(self,deg,x2,y2,x1,y1):
         ...
         a1=list(l1.split(" "))
         #print("angle"+str(deg))
         #if deg>=170 and deg<=180:
         #    print(deg)
         if int(a1[1])<335 and turn==0: #initial limit=330
             ...
             j=j+1
         elif int(a1[1])>=335 and deg<175 and turn==0:#changed #initial limit=330
             print("y = "+str(a1[1]))
             #self.stop()
             self.msg.linear.x = 0.0
             self.msg.linear.y = 0.0
             ...
             try:
                 ...
                 print(e)
         elif int(a1[0])<=85+100 and flag==0:
             self.msg.linear.x = 0.0
             self.msg.linear.y = 0.0
             self.msg.linear.z = 0.0
             self.msg.angular.z = 0.0
             ...
             try:
                 ...
                 print(e)
         elif turn==1 and turn_fl1==0 and int(a1[1])>list1[1][1]-66 and deg>=175: #changed
             turn_fl1=1 
             print(str(a1[1])+"and angle="+str(deg))       
